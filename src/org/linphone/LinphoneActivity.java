			...
			if(object.getClass().isArray()) {//Handle Arrays
				...
				for(int i=0; i< Array.getLength(object); i++) {
					if (Array.get(object, i) instanceof LinphoneAuthInfo) {
						...
						string = string+"\nisPayloadTypeEnabled,"+LC_Object_to_String(getLc().isPayloadTypeEnabled((PayloadType) object));
						string = string+"\npayloadTypeIsVbr,"+LC_Object_to_String(getLc().payloadTypeIsVbr((PayloadType) object));
					}else {
						try {
